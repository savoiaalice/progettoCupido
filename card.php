<?php
$fotos    = $fotos    ?? [];
$nome     = $nome     ?? '';
$cognome  = $cognome  ?? '';
$eta      = $eta      ?? '';
$citta    = $citta    ?? '';
$id_altro = $id_altro ?? null;
?>

<!DOCTYPE html>
<html lang="it">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Cupido - Match</title>
    <!--importo il font-->
    <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@300;400;600;700&display=swap" rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/font/bootstrap-icons.css">
    
    <style>
        :root {
            --primary-color: #c62874;
            --accent-color: #fce4ec;
            --text-main: #333;
        }

        body {
            background-color: var(--accent-color);
            font-family: 'Monteserrat';
        }

        .main-wrapper {
            min-height: 100vh;
            display: flex;
            align-items: center;
            justify-content: center;
            padding: 2rem 0; /* Spazio extra per scroll su schermi piccoli */
        }

        .hero-section {
            background: linear-gradient(rgba(198, 40, 116, 0.6), rgba(0, 0, 0, 0.6)), 
                        url('https://images.unsplash.com/reserve/Af0sF2OS5S5gatqrKzVP_Silhoutte.jpg?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D');
                        /*foto della pagina di apertura*/
            background-size: cover;
            background-position: center;
            color: white;
            padding: 3rem;
        }

        .auth-section {
            background-color: #ffffff;
            padding: 3rem;
        }


        .content-box {
            border-radius: 20px;
            overflow: hidden;
            box-shadow: 0 15px 35px rgba(0,0,0,0.1);
        }
        .btn-outline-custom{
            color: var(--primary-color);
            border: 2px solid var(--primary-color);
            font-weight: 600;
        }
        .btn-outline-custom:hover{
            background-color: #a31f5f;
            color:white;
            border-color: #a31f5f;
        }

        .card-foto {
            width: 100%;
            height: 420px;          /* riquadro fisso */
            overflow: hidden;
            border-radius: 15px;
            position: relative;
        }

        .card-foto img {
            width: 100%;
            height: 100%;
            object-fit: cover;      /* riempie senza deformare */
            object-position: center;
        }

        .carousel-item {
            height: 420px;          /* altezza fissa del riquadro */
        }

        .carousel-item img {
            width: 100%;
            height: 100%;
            object-fit: cover;      /* riempie il riquadro senza deformare */
            object-position: center; /* centra il volto */
            border-radius: 15px;
            object-fit: contain;
            background-color: #fce4ec;
        }

        .btn-like, .btn-skip {
            width: 70px;
            height: 70px;
            border-radius: 50%;
            font-size: 30px;
            display: flex;
            align-items: center;
            justify-content: center;
        }

        .btn-like {
            background-color: #a31f5f;
            color: white;
        }

        .btn-skip {
            background-color: #ddd;
            color: #fd0505;
        }
        .mt-3{
            font-family: 'Monteserrat';
        }
    </style>
</head>

<body>
    <!-- Contenuto che verrà aggiornato con i match -->
    <div class="container-fluid, col-8, aligned-center" style="padding-bottom: 80px;">
    <div class="card p-3">

        <!-- FOTO / CAROUSEL -->
        <div class="card-foto">
            <div id="carouselUtente" class="carousel slide" data-bs-ride="carousel">
                <div class="carousel-inner">

                    <?php
                    $active = "active";
                    foreach ($fotos as $foto) {
                        echo '
                        <div class="carousel-item '.$active.'">
                            <img src="'.$foto['percorso'].'" alt="foto utente">
                        </div>';
                        $active = "";
                    }
                    ?>

                </div>

                <button class="carousel-control-prev" type="button" data-bs-target="#carouselUtente" data-bs-slide="prev">
                    <span class="carousel-control-prev-icon"></span>
                </button>

                <button class="carousel-control-next" type="button" data-bs-target="#carouselUtente" data-bs-slide="next">
                    <span class="carousel-control-next-icon"></span>
                </button>
            </div>
        </div>

        <!-- INFO UTENTE -->
        <div class="mt-3">
            <h3 class="fw-bold"><?php echo $nome . " " . $cognome; ?>, <?php echo $eta; ?></h3>
            <p class="text-muted"><?php echo $citta; ?></p>

            <a href="profiloUtente.php?id=<?php echo $id_altro; ?>" class="btn btn-outline-custom w-100 mt-2">
                Vedi profilo completo
            </a>
        </div>

        <!-- BOTTONI LIKE / SKIP -->
        <div class="d-flex justify-content-around py-3">
            <a href="azione.php?azione=skip&id=<?php echo $id_altro; ?>" class="btn-skip">
                <i class="bi bi-x-lg"></i>
            </a>

            <a href="azione.php?azione=like&id=<?php echo $id_altro; ?>" class="btn-like">
                <i class="bi bi-heart-fill"></i>
            </a>
        </div>

    </div>
</div>


    <!--Barra che si trova in basso con le icone -->
    <nav class="navbar fixed-bottom bg-white border-top">
    <div class="container-fluid">
        <div class="row text-center w-100">

            <div class="col">
                <a href="home.php" class="text-decoration-none text-dark">
                    <?php include "cupido.php"; ?>
                </a>
            </div>
            <div class="col">
                <a href="search.php" class="text-decoration-none text-dark">
                    <i class="bi bi-search-heart fs-3" style="color:#a31f5f;"></i>
                </a>
            </div>

            <div class="col">
                <a href="match.php" class="text-decoration-none text-dark">
                    <i class="bi bi-chat-heart fs-3" style="color:#a31f5f;"></i>
                </a>
            </div>

            <div class="col">
                <a href="profilo.php" class="text-decoration-none text-dark">
                    <i class="bi bi-person-circle fs-3" style="color:#a31f5f;"></i>
                </a>
            </div>
            </div>
    </div>
</nav>

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>